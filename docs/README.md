# Documentation du projet — Pilulier connecté

Projet intégrateur 420-321-AH
Équipe : Rauf Mifteev, Daniel Alain Dyky, Chahrazad Tayibi

Un pilulier de 28 cases qui vérifie par image qu'un médicament a bien été retiré, et qui guide le patient avec une lumière sur la bonne case.

Un fichier par livrable. Le nom du fichier commence par le code du livrable.

## A — Cadrage du projet

- **A1** — [Périmètre arrêté](A1-perimetre.md)
- **A2** — [Analyse des besoins](A2-analyse-des-besoins.md)
- **A3** — [Cahier des charges](A3-cahier-des-charges.md)
- **A4** — [Dimension intelligente](A4-dimension-intelligente.md)
- **A5** — [Risques](A5-risques.md)

## B — Modélisation et conception

- **B1** — [Diagramme de cas d'utilisation](B1-cas-utilisation.md)
- **B2** — [Scénarios d'utilisateurs](B2-scenarios-utilisateurs.md)
- **B3** — [Diagramme de classes](B3-diagramme-classes.md)
- **B4** — [Diagrammes de séquence](B4-diagrammes-sequence.md)
- **B5** — [Architecture logicielle](B5-architecture-logicielle.md)
- **B6** — [Patrons de conception](B6-patrons-conception.md)
- **B7** — [Choix technologiques](B7-choix-technologiques.md)
- **B8** — [Maquettes](B8-maquettes.md)

## C — Carnet de produit et planification

- **C1** — [Carnet de produit](C1-carnet-produit.md)
- **C2** — [Planification du sprint 1](C2-sprint-1.md)

## D — Conventions de l'équipe

- **D1** — [Contrat d'équipe](D1-contrat-equipe.md)
- **D2** — [Définition de « prêt »](D2-definition-pret.md)
- **D3** — [Définition de « terminé »](D3-definition-termine.md)
- **D4** — [Stratégie de tests](D4-strategie-tests.md)
- **D5** — [Plan de documentation](D5-plan-documentation.md)

## E — Environnement de travail

- **E1** — [Dépôt de code](E1-depot-code.md)
- **E2** — [Projet de gestion](E2-projet-gestion.md)
- **E3** — [Espace de documentation](E3-espace-documentation.md)

## Images

Toutes les images vont dans le dossier `assets/`, sans les renommer. Les documents pointent déjà vers ces noms.

| Fichier attendu | Utilisé par |
|---|---|
| `Diagramme_de_cas_d_utilisation.png` | B1 |
| `Diagramme_de_classes.png` | B3 |
| `Sequence_configuration_initiale.png` | B4 |
| `Sequence_remplissage_hebdomadaire.png` | B4 |
| `Sequence_prise_normale.png` | B4 |
| `Sequence_prise_ambigue.png` | B4 |
| `Sequence_prise_manquee.png` | B4 |
| `Architecture_logicielle.png` | B5 |
| `diagramme-Gantt.png` | A3 |

Maquettes à produire, à déposer dans `assets/` sous ces noms :

`01-connexion.png` · `02-tableau-de-bord.png` · `03-creneaux.png` · `04-medicaments.png` · `05-remplissage.png` · `06-resultat.png` · `07-historique.png` · `08-demonstration.png`

Les sources PlantUML et Mermaid vont dans `assets/sources/`.
